package com.paya.paragon.utilities.CountryCode;
...
class Country implements Comparable<Country> {
    ...
    private static CountryCodePicker.Language loadedLibraryMasterListLanguage;
    ...
    public static void loadDataFromXML(Context context, CountryCodePicker.Language language) {
        ...
        try {
            ...
            int value = context.getResources().getIdentifier(language.toString().toLowerCase(), "raw", context.getPackageName());
            if(value > 0){
                InputStream ins = context.getResources().openRawResource(value);
    ...
    public boolean isEligibleForQuery(String query) {
        if (query != null && !query.equals("")) {
            ...
            if (getEnglishName() != null) {
                return getEnglishName().toLowerCase().contains(query);
            }
            return true;
           /* return (getName().toLowerCase().contains(query) ||
