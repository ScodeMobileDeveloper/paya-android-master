...
package com.paya.paragon.utilities.custompricerange;
...
import com.paya.paragon.utilities.PixelUtil;
...
public class RangeSeekBar<T extends Number> extends View {
    ...
    protected synchronized void onDraw(Canvas canvas) {
       ...
       // canvas.save();
        boolean isRtl = getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
        /*if (isRtl) {
            canvas.scale(-1f, 1f, (float) getWidth() / 2, (float) getHeight() / 2);
        }*/
        ...
        if(isRtl){
            mRect.left = getWidth() - padding;
            mRect.right = padding;
        }else{
            mRect.right = getWidth() - padding;
            mRect.left = padding;
        }
        //Paint for unselected seek bar
        Paint unSelectedSeekBarPaint = new Paint();
        unSelectedSeekBarPaint.setColor(ContextCompat.getColor(getContext(), R.color.color_hint));
        canvas.drawRect(mRect, unSelectedSeekBarPaint);
        ...
        if(isRtl){
            mRect.left = normalizedToScreen(normalizedMaxValue);
            mRect.right = normalizedToScreen(normalizedMinValue);
        }else{
        ...
        //Paint for selected seek bar
        Paint selectedSeekBarPaint = new Paint();
        selectedSeekBarPaint.setColor(ContextCompat.getColor(getContext(), R.color.yellow));
        selectedSeekBarPaint.setStrokeCap(Paint.Cap.ROUND);
        selectedSeekBarPaint.setStrokeWidth(50);
        selectedSeekBarPaint.setAntiAlias(true);
        canvas.drawRect(mRect, selectedSeekBarPaint);
        canvas.drawRoundRect(mRect, 25, 25, selectedSeekBarPaint);
    ...
    private void drawThumb(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
        Paint seekBarThumbPaint = new Paint();
        seekBarThumbPaint.setColor(ContextCompat.getColor(getContext(), R.color.yellow));
        canvas.drawCircle(screenCoord, 10 + mTextOffset - (PixelUtil.dpToPx(getContext(), LINE_HEIGHT_IN_DP)) / 2f, 20, seekBarThumbPaint);
    }
