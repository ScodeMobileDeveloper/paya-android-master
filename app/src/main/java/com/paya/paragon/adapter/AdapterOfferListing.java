...
public class AdapterOfferListing extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AdapterOfferListing(boolean loadMore,Context context, ArrayList<OfferListItemData> offerListItems) {
        this.mOfferListItems = offerListItems;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ViewHolderMain) {
            ...
            OfferListItemData data = mOfferListItems.get(position);
            ...
            if (data.getUserOfferPrice() != null && !data.getUserOfferPrice().equals("")) {
                String strOfferPrice = mContext.getString(R.string.offer_price) + " " + data.getUserOfferPrice();
                viewHolder.offerPrice.setText(strOfferPrice);
            } else {
    ...
    public int getItemViewType(int position) {
        int a = 0;
        if (position < mOfferListItems.size()) {
            a = 0;
        } else {
            a = 1;
        }
    ...
    public int getItemCount() {
        int size;
        if (loadMore) {
            size = mOfferListItems.size() + 1;
        } else {
            size = mOfferListItems.size();
        }
        return size;
    }
